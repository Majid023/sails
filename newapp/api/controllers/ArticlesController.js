...
module.exports = {
	list:function(req ,res){
        Articles.find({}).exec(function(err,result){
            if (err){
                res.send(500,{error:"Database error"});
            }
            res.view('list',{articles:result});
        });
    },
    ...
    edit:function(req,res){
        Articles.findOne({id: req.params.id}).exec(function(err,result){
            if(err){
                res.send(500,{error:"database error"});
            }
            res.view('edit',{article:result});
        });
    },
